...
```bash
export CID=$(docker run --cpus 1 -tid -v /var/run/docker.sock:/var/run/docker.sock -v $DOCKERBIN:/usr/bin/docker -v $HOME/results-$RANDOM:/DistributedMachineLearningThesis/src/results dmlt) && docker exec -ti $CID /bin/bash
```
